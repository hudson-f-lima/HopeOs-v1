-- HOPE OS V1.0.3 — RPC Permission Lock + idempotency race handling + product inventory
-- Execute depois da 001_init.sql e 002_checkout_close_rpc.sql.
-- Objetivo: impedir execução direta da RPC financeira por public/anon/authenticated.
-- Somente o backend com SUPABASE_SERVICE_ROLE_KEY pode executar checkout_close.
...
-- Trava explícita da função financeira crítica.
-- No Supabase, funções públicas podem ficar executáveis via /rest/v1/rpc por anon/authenticated
-- se não houver REVOKE/GRANT explícito.
